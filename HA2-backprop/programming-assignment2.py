...
result_file = "result/backprop.txt"
...
def compute_neural_net_loss(params, X, y, reg=0.0):
    ...
    #第二层
    scores_2 = z1.dot(W2) + b2
    exp_scores = np.exp(scores_2)
    pros =  exp_scores / np.sum(exp_scores,axis = 1,keepdims=True)

    #求data_loss
    pros_log = -np.log(pros[np.arange(N),y])
    data_loss = np.sum(pros_log) / N
    ...
    dW2 = z1.T.dot(dsoft) + reg*W2
    db2 = np.sum(dsoft,axis=0)

    du1 = np.dot(dsoft,W2.T)
    dz1 = du1 * (scores > 0)
