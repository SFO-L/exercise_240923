...
result_file = "result/softmax_result3.txt"
...
def compute_softmax_loss(W, X, y, reg):
    ...
    #用-log计算损失 求平均
    scores_soft_log = -np.log(scores_soft[np.arange(N),y])
    scores_loss = np.sum(scores_soft_log) / N

    #计算正则化损失
    l2_loss = 0.5 * reg * np.sum(W*W)

    #计算总损失
    loss = scores_loss + l2_loss

    #计算梯度
    # 分数的梯度
    dscores = scores_soft.copy()
...
def train(X, y, Xtest, ytest, learning_rate=1e-3, reg=1e-5, epochs=100, batch_size=20):
    ...
    for epoch in range(max_epochs):
        ...
        if epoch % 10 == 0:
            ...
            result_str += f"Epoch {epoch}: loss = {loss:.2f}, train_acc = {train_acc:.4f}, test_acc = {test_acc:.4f}\n"
...
with open(result_file, 'w') as f:
    f.write("New Samples, Class Predictions:    {}\n".format(predictions))